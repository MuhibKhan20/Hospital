...
const BehriaInclave = () => {
  ...
  useEffect(() => {
    const fetchDepartments = async () => {
      try {
        ...
        // Get departments with IDs except from 8 to 12
        const filteredDepartments = allDepartments.filter(department => !(department.id >= 8 && department.id <= 12));
  ...
  return (
    <>
      <style jsx>{`
        .behria_inclave_header_image {
          background: linear-gradient(180deg, rgba(40, 76, 170, 0.74) 0%, rgba(3, 5, 29, 0.64) 100%), 
                      url("/behria_inclave.jpg") no-repeat center center;
          background-size: cover;
          height: 80vh;
          position: relative;
          z-index: 1;
        }
        .behria_inclave_header_image::after {
          content: "";
          position: absolute;
          top: 0;
          left: 0;
      ...
      <div className="container-fluid position-relative">
        <div className="row">
          <div className="col-12 behria_inclave_header_image d-flex flex-column align-items-center justify-content-center">
            {/* Text Section */}
            <div className='z-1 text-center d-flex flex-column align-items-center' style={{ marginTop: "8rem", '@media (min-width: 576px)': { marginTop: "0.25rem" } }} >'
              <h2 className='fw-bold text-white fs-1' data-aos="fade-up">Behria Inclave Branch</h2>
              <p className='text-white fs-5' data-aos="fade-up" style={{ maxWidth: "55rem" }}>
                The Behria Inclave Branch of Farooq Hospital is dedicated to providing exceptional medical care and services, ensuring the health and well-being of its community.
              </p>
              {/* Buttons Section */}
