  ...
  useEffect(() => {
    const fetchDepartments = async () => {
      try {
        ...
        // Get departments with IDs from 4 to 8
        const filteredDepartments = allDepartments.filter(department => department.id >= 4 && department.id <= 8);
      ...
      <style jsx>{`
        .dha_header_image {
          background: linear-gradient(180deg, rgba(40, 76, 170, 0.74) 0%, rgba(3, 5, 29, 0.64) 100%), 
                      url("/dha.jpg") no-repeat center center;
        ...
        <div className="row">
          <div className="col-12 dha_header_image d-flex flex-column align-items-center justify-content-center">
            ...
            <div className='z-1 text-center d-flex flex-column align-items-center' style={{ marginTop: "8rem", '@media (min-width: 576px)': { marginTop: "0.25rem" } }} >'
              <h2 className='fw-bold text-white fs-1' data-aos="fade-up">DHA Branch</h2>
              <p className='text-white fs-5' data-aos="fade-up" style={{ maxWidth: "55rem" }}>
                The DHA Branch of Farooq Hospital offers state-of-the-art medical facilities and a team of highly skilled professionals dedicated to providing excellent healthcare services to the community.
              </p>
              {/* Buttons Section */}
